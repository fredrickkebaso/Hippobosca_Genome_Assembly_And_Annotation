...
echo Filtering contigs below 89 basepairs
...
genome_file=${results}/hcamelina_m_spades_genome.fa
threads=24
new_file=$(basename $genome_file .fa)

echo Number of original contigs $(grep -c "^>" ${genome_file})

seqkit seq --remove-gaps  --min-len 89 "${genome_file}" > ${results}/${new_file}_filtered_assembly.fa
