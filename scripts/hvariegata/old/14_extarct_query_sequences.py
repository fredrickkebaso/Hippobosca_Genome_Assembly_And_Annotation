...
homolog_file = f"{basedir}/results/blast_homologs/spades"
print("Extracting hits...")
for file in os.listdir(homolog_file):
    if file.endswith("_homolog.txt"):
        # if file.endswith("hits.txt"):
        file_dir = os.path.join(homolog_file, file)
        hits_file,extension=os.path.splitext(os.path.basename(file_dir))
        prot_file=os.path.join(homolog_file, hits_file + "_hits.txt")
