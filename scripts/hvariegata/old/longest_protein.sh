...
gff_file="${basedir}/results/blast_homologs/spades/orthofinder/"
...
# "${basedir}/scripts/gff2gtf.pl"  "${gff_file}" | grep -w "gene" |sed 's/_id ".*"://' |sed 's/transcript_id/\t/' | cut -f1,2,3,4,5,6,7,8,9 > "${results}/genomic.CDS"

echo "Genomic CDS file created,well formated -- Ok"
echo " "
echo "Continuing to filter longest genes from genomic.CDS -- Ok"

for file in "${gff_file}/"*.gff3
do
cgat gtf2gtf -I "${file}" --method=filter --filter-method=gene >> "${gff_file}/genomic.longest_gene.gff"
