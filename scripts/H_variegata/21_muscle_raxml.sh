...
#PBS -o /mnt/lustre/users/fkebaso/hippo/hvariegata_female/results/muscle_raxml/muscle_raxml.out
#PBS -e /mnt/lustre/users/fkebaso/hippo/hvariegata_female/results/muscle_raxml/muscle_raxml.err
#PBS -m abe
...
trimai=("nogaps" "gappyout" "strictplus" "automated1")

for option in ${trimai[@]}; do
    ...
    results="${basedir}/muscle_raxml/${option}"
    ...
    trimal -in "${results}/${aln_file_name}" -out ${results}/trimal_edited_${aln_file_name}  -htmlout ${results}/trimal_stats_${aln_file_name}.html "-${option}"
